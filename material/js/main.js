...
const image = body.querySelector('#key');
...
if(keys == null){
    ...
    h1.textContent = Number(keys).toLocaleString();
}
...
image.addEventListener('click' , (e)=> {
    ...
    if(Number(power) > 0){
        localStorage.setItem('keys' , `${Number(keys) + 1}`);
        h1.textContent = `${(Number(keys) + 1).toLocaleString()}`;
